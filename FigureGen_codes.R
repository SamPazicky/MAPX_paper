...
dir.create("normalization_plots")
datafiles <- list.files(path="../IDC_all/datafiles", pattern="Pf_Proteins.txt", full.names=TRUE)
...
for(df in datafiles) {
  ...
  # can be saved.
  ggsave(paste0("normalization_plots/MCdata_normalization_",tp,"_rep",rep,".png"),MCdata.scaled$plot)
...
saveRDS(all.predictors,"all.predictors.RDS")
saveRDS(all.features,"all.features.RDS")
saveRDS(all.fitdata,"all.fitdata.RDS")
saveRDS(all.scaledata,"all.scaledata.RDS")
saveRDS(all.rawdata,"all.rawdata.RDS")
  ...
  for(rep in reps) {
    ...
    for(btp in 1:nrow(best.train.pars)) {
      ...
      cross.model <- X.crosstrain.tree(data=all.predictors[[tp]][[rep]],standard.set=GS_specific,train.pars=best.train.pars.row,
                                       evaluate=TRUE,plot=TRUE,train.split=4, train.cycles=25,
                                       save.dir=paste0("crossmodels_",tp,"_rep",rep,"_btp",btp))
      ...
      data.table::fwrite(comp.models$data, paste0("crossmodels_",tp,"_rep",rep,"_btp",btp,"/averaged_predictions.csv"))
      ggsave(paste0("crossmodels_",tp,"_rep",rep,"_btp",btp,"/averaged_predictions.png"), comp.models$plot, units="cm", height=8,width=8)
...
mchoice <- fread(paste0(mfolder,"/model_choice.csv"))

anoDB <- fread("anoDB.csv")
...
for(tp in timepoints) {
  ...
  for(rp in reps) {
    ...
    if(is.na(RFno)) {
      ...
      rep.predictions[[rp]] <- fread(paste0(mfolder,"/crossmodels_tp",tp,"_rep",rp,"_btp",RFno,"/averaged_predictions.csv"))
    }
  }
  #averaging of predictions across replicates
  all.predictions[[tp]] <- X.average.reps(data=rep.predictions,
                                          evaluate=TRUE,standard.set=gold.standard)
  
  ggsave(paste0(nfolder,"/averaged.predictions_AUPRC_tp",tp,".png"), all.predictions[[tp]]$plot)
  fwrite((all.predictions[[tp]]$data),  paste0(nfolder,"/averaged.predictions_tp",tp,".csv"))
  print(paste0("Done tp ", tp,"."))
}
...
for(tp in timepoints) {
  ...
  final_networks <- list()
  gc()
  precision_scan <- seq(0.20,0.9,0.02)
  for(ps in precision_scan) {
    ...
    network.built <- X.build.complexes(data=cut_data, algo="SP",scores.col="score",init.stats=TRUE,final.stats=TRUE,
                                       standard.set=gold.standard,labels.col="complex",  SP.finalpreds="original")
    ...
    network.post.split <- X.postprocess(data=network.refined$data, mode="split", scores.col="score", final.stats=TRUE,
                                        standard.set=gold.standard,labels="complex", weighted=FALSE)
    
    network.post.trim <- X.postprocess(data=network.post.split$data, mode="trim", scores.col="score", final.stats=TRUE,
                                       standard.set=gold.standard,labels="complex", weighted=TRUE)
  ...
  saveRDS(final_networks,paste0(nfolder,"/tp",tp,"_final_networks.RDS"))
  ...
  fwrite(choicedata, paste0(nfolder,"/tp",tp,"_cutoff.choice.csv"))
}
...
# The function X.AI will calculate the assembly factors. This takes some time.
data.AIs <- X.AI(data.PCAs$data,MAPX::complexes[focus.complexes], quality.data, trials=250)
...
pdf("tileplot_z_averaged_centered_legend.pdf")
plot(plotlegend)
dev.off()
plot <- plot + theme(legend.position="none")
#Fig4B
ggsave(paste0("tileplot_z_averaged_centered.png"),plot,height=8,width=6,units="cm")
ggsave(paste0("tileplot_z_averaged_centered.svg"),plot,height=8,width=6,units="cm")
...
for(cpx in names(mapx_mls$PCA.plots)) {
  for(tp in str_remove(timepoints,"tp")) {
    for(rep in paste0("rep",reps)) {
      ggsave(paste0("moonlighters/",cpx,"_tp",tp,"_",rep,".png"),mapx_mls$PCA.plots[[cpx]][[tp]][[rep]])
    }
  }
}
...
moonlighting_proteins_plot <- mapx_mls$moonlighters %>%
  ...
  theme(axis.text.y=element_text(size=5,color="black"),
        ...
        strip.text=element_text(size=7,hjust=0,margin=margin(1,0,0,0,"pt")),
...
#Fig5A
ggsave("moonlighting_proteins_all.png",moonlighting_proteins_plot,height=16,width=6,units="cm", dpi=600)
...
#DataS4
pdf("moonlighting_all.pdf", width=8, height=15)
...
rocplots <- list()
for(tp in timepoints) {
  hpi <- as.numeric(str_remove(tp,"tp"))
  # load data
  predictions <- all.predictions %>% filter(condition==hpi)
  
  toevaluate <- cross_join(predictions, GS, vars=paste0("protein",1:2), mode="inner")
  averaged_predictions.roc <- X.evaluate(toevaluate,scores.col="pred",labels.col="complex", eval.metric="roc",plot=TRUE,black=TRUE)
  ...
  averaged_predictions.prc <- X.evaluate(toevaluate,scores.col="pred",labels.col="complex", eval.metric="prc",plot=TRUE,black=TRUE)
  prc.plot <- averaged_predictions.prc %>%
    ...
    scale_y_continuous(breaks=c(0.25,0.5,0.75),expand=c(0.001,0.001))
  curdata <- data.frame(tp=hpi,roc=averaged_predictions.roc$eval.metric,prc=averaged_predictions.prc$eval.metric)
  eval.metric.table <- bind_rows(eval.metric.table,curdata)
  
  #Fig1E
  ggsave(paste0("AUCs/AUROC_",hpi,"hpi.png"),roc.plot,height=9, width=9, units="cm")
  ggsave(paste0("AUCs/AUPRC_",hpi,"hpi.png"),prc.plot,height=9, width=9, units="cm")
  prcplots[[hpi]] <- prc.plot
  rocplots[[hpi]] <- roc.plot
  
}
#Fig1E
fwrite(eval.metric.table, "AUCs/eval.metric.table.csv")
...
datafiles <- list.files(path="final_choices", pattern="_pairwise.csv", full.names=TRUE) %>% gtools::mixedsort()
...
plot_no.proteins <- all.rawdata %>%
  ...
  theme(plot.title=element_text(size=18),
        ...
        legend.position="none")
plot_no.proteins  
ggsave(paste0("Fig/plot_no.proteins.png"),height=9, width=9, units="cm")
...
for(pc in plotcomplexes) {
  plot <- all.scaledata$tp34$`2` %>%
    ...
    customPlot +
    theme(legend.position="none",
          axis.text.y=element_text(size=6, color="black",margin=margin(r=1)),
          axis.text.x=element_text(size=6, color="black",vjust=3),
          axis.title.y=element_text(size=7, color="black",margin=margin(0,0,0,0),vjust=2),
          axis.title.x=element_text(size=7, color="black",margin=margin(0,0,0,0),vjust=2.5),
          panel.border = element_rect(color="black",fill=NA,size=1),
          axis.ticks.length=unit(.025, "cm"),
          axis.ticks=element_line(linewidth=0.25))
  
  ggsave(paste0("Fig/allcurves_jitterplot_",pc,"_tp34_rep3.png"), plot, units="cm", height=3.5,width=3.5)
  ggsave(paste0("Fig/allcurves_jitterplot_",pc,"_tp34_rep3.svg"), plot, units="cm", height=3.5,width=3.5)
}
#Fig1A
...
plot
ggsave(paste0("Fig/allcurves_jitterplot_mixed_tp34_rep3.png"), plot, units="cm", height=12,width=12)
ggsave(paste0("Fig/allcurves_jitterplot_mixed_tp34_rep3.svg"), plot, units="cm", height=12,width=12)
...
for(i in 1:nrow(plotcomplexes)) {
  ...
  for(cl in seq_along(complexlist)) {
    ...
    for(rep in reps) {
      ...
      plot
      #Fig2A #Fig2B
      ggsave(paste0("Fig/individual_complexes/",nowcomplex,"_cl",cl,"_",tp,"_rep",rep,"_meltcurves.png"),plot,width=4,height=4,units="cm")
      ggsave(paste0("Fig/individual_complexes/",nowcomplex,"_cl",cl,"_",tp,"_rep",rep,"_meltcurves.svg"),plot,width=4,height=4,units="cm")
    }
  }
}
...
plot <- all.maps %>%
  ...
  theme_bw() +
  customPlot
ggsave("Fig/histogram_size_of_clusters.png", plot, units="cm", height=9, width=12)
...
plot
#SFig2B
ggsave("Fig/prefoldin_curves.png",plot,width=10,height=6,units="cm")
ggsave("Fig/prefoldin_curves.svg",plot,width=10,height=6,units="cm")
...
for(cpx in names(complexes)) { 
  tileplot_global <- table1_red %>%
    ...
    theme(
      ...
      axis.text.y = element_text(color="black"),
      axis.text.x = element_text(margin=margin(1,0,0,0,"pt"),size=7, color="black"),
      axis.title = element_blank(),
  ...
  ggsave(paste0("Fig/tileplot_",cpx,".png"),width=9, units="cm", dpi=600)
  ggsave(paste0("Fig/tileplot_",cpx,".svg"),width=9, units="cm", dpi=600)
  
}

library(magick)
imgfiles <- list.files("Fig", pattern="^tileplot_.*png$", full.names=TRUE) %>%
  .[!str_detect(.,"cropped")]
for(imgf in imgfiles) {
  tryCatch({
    #Read the image
    image <- image_read(imgf)
    # Crop the white background
    cropped_image <- image_trim(image)
